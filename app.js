'use strict';
function missingFrom(file,checks){const gone=checks.filter(c=>!c[1]).map(c=>c[0]);return gone.length?{file,gone}:null;}
function bootMessage(boot){
 const el=document.querySelector('#main');if(!el)return;
 el.innerHTML=`<section class="panel"><h2>${boot.file} did not load</h2><p>The page found <b>index.html</b>, <b>styles.css</b> and <b>app.js</b>, but <b>${boot.file}</b> is missing or failed to load, so the game cannot start.</p><p class="micro">Not defined: ${boot.gone.join(', ')}</p><h3>If you are a student</h3><p>Show your teacher this message. Nothing you have already saved has been lost.</p><h3>If you deployed this</h3><p>Check that <b>${boot.file}</b> sits next to <b>index.html</b> on the server, and that the filename case matches exactly. Open it directly in the address bar &mdash; if that is a 404, the file was not published. New files need adding to version control before they will deploy.</p></section>`;
 const st=document.querySelector('#save-status');if(st)st.textContent='Not started';
}
const dataBoot=missingFrom('data.js',[['CATEGORIES',typeof CATEGORIES!=='undefined'],['CLUES',typeof CLUES!=='undefined'],['SOURCES',typeof SOURCES!=='undefined'],['BRANDS',typeof BRANDS!=='undefined'],['MISSIONS',typeof MISSIONS!=='undefined'],['DIAGNOSE',typeof DIAGNOSE!=='undefined'],['MODERATION',typeof MODERATION!=='undefined'],['VERDICTS',typeof VERDICTS!=='undefined'],['WRITING',typeof WRITING!=='undefined'],['CONNECTIONS',typeof CONNECTIONS!=='undefined'],['BENEFITS',typeof BENEFITS!=='undefined']]);
if(dataBoot)bootMessage(dataBoot);
if(!dataBoot){
const KEY = 'segment-studio-v1';
const $ = s => document.querySelector(s);
const esc = s => String(s ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const shuffle = a => { const r=[...a]; for(let i=r.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[r[i],r[j]]=[r[j],r[i]];} return r; };
const fresh = () => ({version:3,round:0,order:shuffle(CLUES.map((_,i)=>i)),sortIndex:0,sortAnswers:{},diagnoseIndex:0,diagnoseAnswers:{},diagnoseOrder:DIAGNOSE.map(d=>shuffle(d.options.map((_,i)=>i))),missionIndex:0,missionPicks:{},connections:{},missionOrder:MISSIONS.map(m=>shuffle(m.options.map((_,i)=>i))),missionResults:{},modIndex:0,modVerdicts:{},modReasons:{},modOrder:MODERATION.map(m=>shuffle(m.reasons.map((_,i)=>i))),notes:{siteA:'1',siteB:'4'},checks:{},finished:false,ui:{designStep:0,verdictStep:0,siteTab:'A',connectionKind:'audience'},siteDrafts:{}});
let state=fresh(),storageOK=true,loadWarning='';
try {
 const saved=JSON.parse(localStorage.getItem(KEY));
 if([1,2,3].includes(saved?.version) && Array.isArray(saved.order) && saved.order.length===CLUES.length && new Set(saved.order).size===CLUES.length && saved.order.every(n=>Number.isInteger(n)&&n>=0&&n<CLUES.length)){
  ...
  if(saved.version<3){state.modIndex=0;state.modVerdicts={};state.modReasons={};state.modOrder=MODERATION.map(m=>shuffle(m.reasons.map((_,i)=>i)));loadWarning='The Challenge round is now a moderation task, so that one round has restarted. Everything else you saved is still here.';}
  state.version=3;
  for(const [k,max] of [['round',4],['sortIndex',CLUES.length],['missionIndex',MISSIONS.length],['diagnoseIndex',DIAGNOSE.length],['modIndex',MODERATION.length]])state[k]=Math.max(0,Math.min(max,Number(state[k])||0));
  for(const k of ['sortAnswers','missionPicks','connections','missionResults','diagnoseAnswers','modVerdicts','modReasons','notes','checks','siteDrafts'])if(!state[k]||typeof state[k]!=='object'||Array.isArray(state[k]))state[k]={};
  for(const [k,list,prop]of [['missionOrder',MISSIONS,'options'],['diagnoseOrder',DIAGNOSE,'options'],['modOrder',MODERATION,'reasons']])if(!Array.isArray(state[k])||state[k].length!==list.length||list.some((item,i)=>!Array.isArray(state[k][i])||state[k][i].length!==item[prop].length||new Set(state[k][i]).size!==item[prop].length||state[k][i].some(n=>!Number.isInteger(n)||n<0||n>=item[prop].length)))state[k]=list.map(item=>shuffle(item[prop].map((_,i)=>i)));
...
const roundNames=['Decode','Design','Diagnose','Moderate','The verdict'];
const roundSubs=['Sort audience details','Choose & connect features','Find the performance factor','Judge draft extracts','Compare two websites'];
...
function modDone(i){return state.modVerdicts[i]!==undefined&&state.modReasons[i]!==undefined;}
function modComplete(){return MODERATION.every((_,i)=>modDone(i));}
function modCount(){return MODERATION.filter((_,i)=>modDone(i)).length;}
function modScore(){return MODERATION.reduce((t,m,i)=>t+(state.modVerdicts[i]===m.verdict?4:0)+(state.modReasons[i]===m.correct?6:0),0);}
function modRight(k){return MODERATION.filter((m,i)=>k==='verdict'?state.modVerdicts[i]===m.verdict:state.modReasons[i]===m.correct).length;}
function xp(){return correctSort()*5+Object.values(state.missionResults).reduce((a,b)=>a+(Number(b.score)||0),0)+correctDiagnose()*10+modScore()+(state.finished?100:0);}
function isAllPlayed(){return Object.keys(state.sortAnswers).length===20&&Object.keys(state.missionResults).length===5&&Object.keys(state.diagnoseAnswers).length===8&&modComplete();}
function roundDone(i){return [Object.keys(state.sortAnswers).length===20,Object.keys(state.missionResults).length===5,Object.keys(state.diagnoseAnswers).length===8,modComplete(),state.finished][i];}
...
function syncHud(){
 const counts=[`${Object.keys(state.sortAnswers).length}/20 clues`,`${Object.keys(state.missionResults).length}/5 briefs`,`${Object.keys(state.diagnoseAnswers).length}/8 cases`,`${modCount()}/10 extracts`,state.finished?'Case file complete':`${draftProgress()}/9 entries`];
...
function render(focus=false){syncHud();[renderSort,renderDesign,renderDiagnose,renderModerate,renderVerdict][state.round]();save();if(focus)$('#main').focus({preventScroll:true});}
...
function renderDiagnose(){
 ...
 if(state.diagnoseIndex>=DIAGNOSE.length){
  h+=`<section class="completion"><span class="completion-mark" aria-hidden="true">\u2713</span><div class="eyebrow">CLINIC COMPLETE</div><h3>Symptom, factor, evidence.</h3><p>${correctDiagnose()} of 8 first answers correct. A.M1 and A.D1 both say <em>high-performance</em> \u2014 this is the vocabulary that makes that claim evidenced instead of asserted.</p>${action('Next: moderate the drafts \u2192','goto-claims')}</section><section class="panel"><h3>Carry these into your analysis</h3>${DIAGNOSE.map((d,i)=>`<details ${state.diagnoseAnswers[i]!==d.correct?'open':''}><summary>${state.diagnoseAnswers[i]===d.correct?'\u2713':'\u21ba'} ${esc(d.options[d.correct].split('\u2014')[0].trim())} <span class="detail-score">${d.where}</span></summary><p>${esc(d.symptom)}</p><div class="design-takeaway"><b>Why</b><span>${esc(d.why)}</span></div><div class="tradeoff"><b>Confirm it</b> ${esc(d.confirm)}</div></details>`).join('')}</section>`;
 }else{
...
function biasNote(){
 const chose=[0,1,2].map(v=>MODERATION.filter((_,i)=>state.modVerdicts[i]===v).length);
 const actual=[0,1,2].map(v=>MODERATION.filter(m=>m.verdict===v).length);
 const top=chose.indexOf(Math.max(...chose)),over=chose[top]-actual[top];
 const line=over<3
  ?'Your verdicts came out spread much like the extracts themselves. That is the habit worth keeping \u2014 judge each one on what it actually says.'
  :[`You called ${chose[0]} of 10 \u201csound\u201d when ${actual[0]} were. If nothing needs work, there is nothing to analyse \u2014 and A.M1 marks the analysis.`,
    `You called ${chose[1]} of 10 \u201chalf-way\u201d when ${actual[1]} were. \u201cNeeds more evidence\u201d is the safe answer that avoids committing. Some extracts are simply wrong; some are simply fine.`,
    `You called ${chose[2]} of 10 \u201cnot yet\u201d when ${actual[2]} were. Challenging everything is the mirror image of accepting everything \u2014 A.D1 asks you to reach a judgement, not to withhold one.`][top];
 return `<section class="panel bias"><div class="section-heading"><div><span class="eyebrow">YOUR MODERATING</span><h3>How you judged, against how the extracts actually were.</h3></div><span class="count-chip">${modRight('verdict')}/10 verdicts \u00b7 ${modRight('reason')}/10 reasons</span></div><div class="bias-grid">${VERDICTS.map((vd,n)=>`<div><b>${vd.label}</b><span>you said <strong>${chose[n]}</strong></span><span>actually <strong>${actual[n]}</strong></span></div>`).join('')}</div><p>${line}</p></section>`;
}
function renderModerate(){
 let h=header(4,'Sound, half-way, or not yet?','Moderate ten extracts from a draft report. Give a verdict, then say why. Some of them are fine as written.','10 extracts \u00b7 100 XP');
 if(state.modIndex>=MODERATION.length){
  h+=`<section class="completion"><span class="completion-mark" aria-hidden="true">\u2713</span><div class="eyebrow">MODERATION COMPLETE</div><h3>${modScore()} / 100 XP.</h3><p>Verdict 4 XP, reason 6 XP. The reason is worth more because knowing that something is weak is easier than knowing what it needs.</p>${action('Next: build your two-site verdict \u2192','goto-verdict')}</section>`+biasNote()+`<section class="panel"><h3>Carry these into your own writing</h3>${MODERATION.map((m,i)=>`<details ${!modDone(i)||state.modReasons[i]!==m.correct?'open':''}><summary>${state.modReasons[i]===m.correct&&state.modVerdicts[i]===m.verdict?'\u2713':'\u21ba'} ${m.source} <span class="detail-score">${VERDICTS[m.verdict].label}</span></summary><blockquote>${esc(m.extract)}</blockquote><div class="design-takeaway"><b>Verdict</b><span>${esc(m.why)}</span></div><div class="tradeoff"><b>The lesson</b> ${esc(m.teach)}</div></details>`).join('')}</section>`;
 }else{
  const i=state.modIndex,m=MODERATION[i],v=state.modVerdicts[i],r=state.modReasons[i];
  h+=meter('Extract',i,10,'4 XP for the verdict, 6 for the reason');
  h+=`<div class="question-stage"><div class="missioncard"><div class="card-top"><span class="brandpill">${m.source}</span><span class="scenario-label">Draft report extract</span></div><h3>${esc(m.extract)}</h3></div>`;
  h+=`<div class="section-heading"><div><span class="eyebrow">STEP 1 \u00b7 VERDICT</span><h3>How far does this extract get?</h3></div></div><div class="verdict-scale">${VERDICTS.map((vd,n)=>`<button data-verdict="${n}" class="${v===n?'selected':''} ${v!==undefined&&n===m.verdict?'correct':''}" ${v!==undefined?'disabled':''} aria-pressed="${v===n}"><kbd>${n+1}</kbd><b>${vd.label}</b><small>${vd.cue}</small>${v!==undefined&&n===m.verdict?'<span class="result-label">\u2713 Agreed verdict</span>':''}</button>`).join('')}</div>`;
  if(v===undefined)h+='<div class="answer-hint"><span>Choose a verdict, or use keys <kbd>1</kbd>\u2013<kbd>3</kbd>. Not everything here is faulty.</span><button class="textbutton" data-action="guide">Need the criteria?</button></div>';
  else{
   h+=`<div class="feedback ${v===m.verdict?'':'bad'}" role="status"><div class="feedback-title"><span class="feedback-icon">${v===m.verdict?'\u2713':'\u21ba'}</span><h3>${v===m.verdict?'Verdict agreed \u00b7 +4 XP':`Moderated as \u201c${VERDICTS[m.verdict].label}\u201d`} <small>you said \u201c${VERDICTS[v].label}\u201d</small></h3></div><p>${esc(m.why)}</p></div>`;
   h+=`<div class="section-heading"><div><span class="eyebrow">STEP 2 \u00b7 REASON</span><h3>${v===m.verdict?'Now say why.':'Now work out why it lands there.'}</h3><p class="crit-spec">Worth 6 XP. This is the part that transfers to your own draft.</p></div></div><div class="claimoptions">${state.modOrder[i].map((j,n)=>`<button data-reason="${j}" class="${r===j?'selected':''} ${r!==undefined&&j===m.correct?'correct':''}" ${r!==undefined?'disabled':''}><kbd>${n+1}</kbd><span>${esc(m.reasons[j])}</span>${r!==undefined&&j===m.correct?'<span class="result-label">\u2713 Best reason</span>':''}</button>`).join('')}</div>`;
   if(r!==undefined)h+=`<div class="feedback ${r===m.correct?'':'bad'}" role="status"><div class="feedback-title"><span class="feedback-icon">${r===m.correct?'\u2713':'\u21ba'}</span><h3>${r===m.correct?'Reason agreed \u00b7 +6 XP':'Not the strongest reason'}</h3></div><div class="design-takeaway"><b>The lesson</b><span>${esc(m.teach)}</span></div><div class="actionrow">${action(i===9?'See how you moderated \u2192':'Next extract \u2192','next-moderate')}</div></div>`;
   else h+='<div class="answer-hint">Choose the reason, or use keys <kbd>1</kbd>\u2013<kbd>3</kbd>.</div>';
  }
  h+='</div>';
 }
 $('#main').innerHTML=h;
...
function renderVerdict(){
 ...
 if(step===0){
  ...
  if(state.finished){h+=`<section class="completion"><span class="completion-mark">✓</span><div class="eyebrow">CASE FILE COMPLETE</div><h3>Your verdict is ready to share.</h3><p>${xp()} / 500 agency XP. Download your case file for your teacher or partner to review.</p><div class="badge-row"><span class="badge">Case file complete</span>${correctSort()>=16?'<span class="badge">Audience decoder</span>':''}${modRight('reason')>=6?'<span class="badge">Moderator</span>':''}</div>${action('Download your case file ↓','export')}<p class="micro">${isAllPlayed()?'All five rounds completed.':'You can still finish the earlier game rounds.'} Game XP is not a BTEC grade.</p></section>`;}
...
function exportFile(){
 ...
 lines.push('','ROUND 4 — MODERATE (draft extracts; verdict 4 XP + reason 6 XP)',`Verdicts agreed: ${modRight('verdict')}/10 | Reasons agreed: ${modRight('reason')}/10 | ${modScore()}/100 XP`,'Verdicts chosen vs the key: '+VERDICTS.map((vd,n)=>`${vd.label} ${MODERATION.filter((_,i)=>state.modVerdicts[i]===n).length} (key ${MODERATION.filter(m=>m.verdict===n).length})`).join(' | '));
 MODERATION.forEach((m,i)=>{if(state.modVerdicts[i]===undefined)return;lines.push('',m.source+': '+m.extract,`  Your verdict: ${VERDICTS[state.modVerdicts[i]].label} | Moderated: ${VERDICTS[m.verdict].label}`,'  Why: '+m.why,`  Your reason: ${state.modReasons[i]!==undefined?m.reasons[state.modReasons[i]]:'not chosen'}`,'  Best reason: '+m.reasons[m.correct],'  Lesson: '+m.teach);});
...
function dispatchAction(act){
 switch(act){
  ...
  case 'next-moderate':if(state.modIndex<MODERATION.length&&modDone(state.modIndex)){state.modIndex++;moveView();}break;
...
document.addEventListener('click',e=>{
 ...
 if(d.verdict!==undefined){const i=state.modIndex,a=Number(d.verdict);if(state.round===3&&MODERATION[i]&&VERDICTS[a]&&state.modVerdicts[i]===undefined){state.modVerdicts[i]=a;render();showFeedback();}return;}
 if(d.reason!==undefined){const i=state.modIndex,a=Number(d.reason);if(state.round===3&&MODERATION[i]?.reasons[a]&&state.modVerdicts[i]!==undefined&&state.modReasons[i]===undefined){state.modReasons[i]=a;render();showFeedback();}return;}
...
document.addEventListener('keydown',e=>{
 ...
 if(state.round===3&&n>=0&&n<3&&/^\d$/.test(e.key)){e.preventDefault();const mi=state.modIndex;
  if(state.modVerdicts[mi]===undefined)$(`[data-verdict="${n}"]`)?.click();
  else if(state.modReasons[mi]===undefined)$(`[data-reason="${state.modOrder[mi]?.[n]}"]`)?.click();}
});
...
const boot=missingFrom('criteria.js',[['PRINCIPLES',typeof PRINCIPLES!=='undefined'],['PERF_FACTORS',typeof PERF_FACTORS!=='undefined'],['SITE_REQS',typeof SITE_REQS!=='undefined'],['MOVES',typeof MOVES!=='undefined'],['EXEMPLARS',typeof EXEMPLARS!=='undefined'],['checkMoves',typeof checkMoves!=='undefined'],['checkEvidence',typeof checkEvidence!=='undefined'],['principlesUsed',typeof principlesUsed!=='undefined'],['perfUsed',typeof perfUsed!=='undefined'],['movesMet',typeof movesMet!=='undefined']]);
if(boot){
 bootMessage(boot);
}else{
 render();
